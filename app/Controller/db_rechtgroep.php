...
class DbRechtGroep extends Database {
    ...
    public function getRechtGroepBeschrijving() {
        return $this->rechtgroep_beschrijving;
    }
    
    // Setters
    public function setRechtGroepId($recht_groep_id) {
        $this->rechtgroep_id = $recht_groep_id;
    }
    /**
     *
    ...
    public function setRechtGroepBeschrijving($rechtgroep_beschrijving) {
        
        //Check input
        if (!empty($rechtgroep_beschrijving) ){
            
            // Get rid of additional leading and trailing spaces
            $this->rechtgroep_beschrijving = trim($rechtgroep_beschrijving);
        }
    }
    ...
    public function getRechtGroep(){
        ...
        */
        ...
        $query = "SELECT * FROM  `rechtgroep` LEFT JOIN `recht_bitfield_parts`".
                        " ON  `rechtgroep`.`rechtgroep_id` = `recht_bitfield_parts`.`recht_groep_id`".
                        " WHERE `rechtgroep`.`rechtgroep_id` = '" . $this->dbInString($this->rechtgroep_id) . "'".
    ...
    public function getRechtGroepList(){
         ...
         */
        $query = "SELECT * FROM  `rechtgroep` ".
                        "ORDER BY `rechtgroep_beschrijving`";
    ...
    public function groupNameExists($name){
        ...
        $query = "SELECT * FROM `rechtgroep` WHERE `rechtgroep`.`rechtgroep_beschrijving` ='" .$this->dbInString(trim($name) )."'";
    ...
    public function groupExists($id){
        ...
        $query = "SELECT * FROM `rechtgroep` WHERE `rechtgroep`.`rechtgroep_id` = $id";
    ...
    public function saveRechtGroep($recht_bitfield){
        ...
        $insertquery = "INSERT INTO rechtgroep (recht_bitfield, rechtgroep_beschrijving)
        ...
        if($this->dbquery($insertquery)){
            ...
            $this->recht_groep_id = mysqli_insert_id();
            return true;
    ...
    /**
     *
     * @param type $mysqli_list_data
     * @return array 
     */
    private function dataListDB2Applictie( $mysqli_list_data ){
        ...
        foreach( $mysqli_list_data as $idx => $data_row){
     ...
     */
    private function dataDB2Applictie($mysqli_data_array){
        ...
        foreach($mysqli_data_array as $row_idx => $data){
            ...
            */
            if ( $row_idx == 0 ){
                ...
                foreach( $data as $kolomname => $kolomdata ){
                    ...
                    if ( ($kolomname != 'recht_bitfield_parts_id') 
